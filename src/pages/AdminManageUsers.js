...
function AdminManageUsers() {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        const response = await axios.get('InsertIPHERE:5000/api/users');
  ...
  const handleSaveEdit = async () => {
    try {
      ...
      const response = await axios.put(`InsertIPHERE:5000/api/edit/users/${selectUserId}`, {
  ...
  const handleDelete = async (userId) => {
    if (deleteConfirmationId === userId) {
      try {
        ...
        const response = await axios.put(`InsertIPHERE:5000/api/users/delete/${userId}`);
        console.log(response.data.message); // Log success message
